...
#
#
#  Related Topics 树 深度优先搜索 并查集 动态规划 👍 32 👎 0
import collections
...
class Solution:
    def smallestMissingValueSubtree(self, parents: List[int], nums: List[int]) -> List[int]:
        ...
        vis = [False] * 100002
        u, v, cur = nums.index(1), nums.index(1), 1
        while u != -1:
            dq = collections.deque([w for w in g[u] if w != v])
            vis[nums[u]] = True
            while dq:
                node = dq.popleft()
                vis[nums[node]] = True
...
print(Solution().smallestMissingValueSubtree([-1,2,3,0,2,4,1], [2,3,4,5,6,7,8]))
