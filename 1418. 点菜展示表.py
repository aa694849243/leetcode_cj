...
class Solution:
    def displayTable(self, orders: List[List[str]]) -> List[List[str]]:
        foods=set()
        for n,t,food in orders:
            foods.add(food)
        foods=sorted(list(foods))
        f={food:i for i,food in enumerate(foods)}
        ans=[['Table']+foods]
        m=collections.defaultdict(lambda:[0]*len(foods))
        for n,t,food in orders:
            m[t][f[food]]+=1
        for t in sorted(m,key=lambda x:int(x)):
            m[t]=list(map(str,m[t]))
            ans.append([t]+m[t])
        return ans