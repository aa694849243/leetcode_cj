...
class Solution:
    def isValid(self, code: str) -> bool:
        status = {
            'start': {'start_ltag'},
            'start_ltag': {'start_ltag', 'r_tag'},
            'r_tag': {'start_ltag', 'end_ltag', 'start_cdata'},
            'start_cdata': {'start_cdata', 'end_cdata'},
            # 'end_cdata': {'r_tag', 'end_ltag'},
            'end_ltag': {'end_ltag', 'start_ltag'}
        }
        p = 'start'
        stk = []
        temp = ''
        ...
        while i < len(code):
            ...
            if p == 'start':
                ...
                if ch == '<' and i + 1 < len(code) and code[i + 1] == '/':
                    p = 'end_ltag'
                    i += 2
                elif ch == '<' and i + 8 < len(code) and code[i:i + 9] == '<![CDATA[':
                    p = 'start_cdata'
                    i += 9
                elif ch == '<':
                    p = 'start_ltag'
                ...
                if ch == ']' and i + 2 < len(code) and code[i:i + 3] == ']]>':
                    p = 'r_tag'
                    i += 3
                else:
                    p = 'start_cdata'
