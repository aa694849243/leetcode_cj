# -*- coding: utf-8 -*-
# 一个观光景点由它的名字 name 和景点评分 score 组成，其中 name 是所有观光景点中 唯一 的字符串，score 是一个整数。景点按照最好到最坏
# 排序。景点评分 越高 ，这个景点越好。如果有两个景点的评分一样，那么 字典序较小 的景点更好。
#
...
#  总共 调用 add 和 get 不超过 4 * 10⁴
#
#
#  Related Topics 设计 数据流 有序集合 堆（优先队列）
#  👍 25 👎 0
...
class SORTracker:
    ...
    def __init__(self):
        self.lst = SortedList(key=lambda x: (-x[1], x[0]))
        self.cnt = -1

    def add(self, name: str, score: int) -> None:
        self.lst.add((name, score))

    def get(self) -> str:
        self.cnt += 1
        return self.lst[self.cnt][0]
