...
class DiningPhilosophers:  # 如果全都同时拿起左边的筷子，那么会发生死锁
    def __init__(self):
        self.li = [threading.RLock()] * 5
    ...
    def wantsToEat(self,
                   ...
                   putLeftFork: 'Callable[[], None]',
                   putRightFork: 'Callable[[], None]') -> None:
        left, right = self.li[(philosopher + 1) % 5], self.li[philosopher]
        if philosopher % 2:  # 奇数编号先拿左边叉子，可以设置后拿右边筷子
            left.acquire()
            pickLeftFork()
            pickRightFork()
            eat()
...
class DiningPhilosophers:  # 如果全都同时拿起左边的筷子，那么会发生死锁
    def __init__(self):
        self.li = [threading.RLock()] * 5
        self.phi_meal = threading.Semaphore(4)  # 限制进餐人数为4
    ...
    def wantsToEat(self,
                   ...
                   putLeftFork: 'Callable[[], None]',
                   putRightFork: 'Callable[[], None]') -> None:
        self.phi_meal.acquire()
        self.li[philosopher%5].acquire() #左边筷子锁住
        self.li[(philosopher+1)%5].acquire() #右边筷子锁住
        pickLeftFork()
        pickRightFork()
        eat()
        putLeftFork()
        putRightFork()
        self.li[philosopher%5].release() #左边筷子释放
        self.li[(philosopher+1)%5].release() #右边筷子释放
        self.phi_meal.release()
