...
class Solution:
    def removeComments(self, source: List[str]) -> List[str]:
        ...
        def rec(a, s, flag):
            if len(s) == 0:
                return a, flag
            elif len(s) == 1 and flag == 0:
                return a+s, flag
            i = 1
            while i < len(s):
                if s[i - 1] + s[i] == '//' and not flag:
                    return s[:i - 1], 0
                elif s[i - 1] + s[i] == '/*' and not flag:
                    flag = 1
                    i += 2
                elif s[i - 1] + s[i] == '*/' and flag == 1:
                    a_, flag = rec('', s[i + 1:], 0)
                    return a + a_, flag
                elif not flag:
                    a += s[i - 1]
...
Solution().removeComments(["main() { ", "  int a = 1; /* Its comments here ", "", "  ", "  */ return 0;", "} "])
