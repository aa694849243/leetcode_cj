...
#
#
#  Related Topics 栈 数学 字符串 动态规划 👍 21 👎 0
...
class Solution:
    def minOperationsToFlip(self, expression: str) -> int:
        def calc_and(x1, y1, x2, y2):
            return min(x1 + x2, x1 + y2, x2 + y1), y1 + y2

        def calc_or(x1, y1, x2, y2):
        ...
        def digestion():
            if len(stack_num) >= 2 and stack_op and stack_op[-1] in '&|':
                x1, y1 = stack_num.pop()
                x2, y2 = stack_num.pop()
                op = stack_op.pop()
                x_and, y_and = calc_and(x1, y1, x2, y2)
                x_or, y_or = calc_or(x1, y1, x2, y2)
                if op == '&':
                    stack_num.append((min(x_and, x_or+1), min(y_and, y_or+1)))
                else:
                    stack_num.append((min(x_and+1, x_or), min(y_and+1, y_or)))
