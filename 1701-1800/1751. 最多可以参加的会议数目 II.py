...
#
#
#  Related Topics 数组 二分查找 动态规划 👍 57 👎 0
...
class Solution:
    def maxValue(self, events: List[List[int]], k: int) -> int:
        ...
        for k_ in range(2, k + 1):
            tmp_dp = []
            for s, e, v in events:
                r = bisearch_left(0, len(tmp_dp), s)
                if not tmp_dp:
                    tmp_dp.append([e, v])
                    continue
                if r - 1 < 0:
                    tmp_dp.append(max(tmp_dp[-1],dp[len(tmp_dp)], key=lambda x: x[1]))
                else:
                    x = max(tmp_dp[-1][1], dp[r - 1][1] + v)
                    ma = max(x, ma)
                    tmp_dp.append([e, x])
            dp = tmp_dp
        return ma
...
print(Solution().maxValue([[19,42,7],[52,73,84],[84,92,96],[22,74,44]], 3))
