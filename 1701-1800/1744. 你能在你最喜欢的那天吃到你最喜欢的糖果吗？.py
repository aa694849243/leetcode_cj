...
class Solution:
    def canEat(self, candiesCount: List[int], queries: List[List[int]]) -> List[bool]:
        prefix = [0] + [*itertools.accumulate(candiesCount)]
        # n = len(prefix)
        ans = []

        def cal(li):
            mi, ma = li[1] + 1, (li[1] + 1) * li[2]
            a=bisect.bisect_left(prefix,mi)-1
            b=bisect.bisect_left(prefix,ma)-1
            return a<=li[0]<=b
...
Solution().canEat([5, 2, 6, 4, 1], [[3, 1, 2], [4, 10, 3], [3, 10, 100], [4, 100, 30], [1, 3, 1]])
