...
# - `2 <= jump[i] <= 10^6`
# - `1 <= inc, dec, cost[i] <= 10^6` Related Topics 记忆化搜索 数组 动态规划
#  👍 27 👎 0
    ...
    def busRapidTransit(self, target: int, inc: int, dec: int, jump: List[int], cost: List[int]) -> int:
        ...
        def f(p):
            ...
            for i, val in enumerate(jump):
                if p % val == 0:
                    ans = min(ans, min(cost[i],(p-p//val)*inc) + f(p // val))
                else:
                    rem = p % val
                    if p >= val:
                        ans=min(ans,min(cost[i]+rem*inc,(p-p//val)*inc)+f(p//val))
                    ans=min(ans,min(cost[i]+(val-rem)*dec,(p-p//val-1)*inc)+f(p//val+1))
            return ans
        return f(target)%mod
Solution().busRapidTransit(target = 612, inc = 4, dec = 5, jump = [3,6,8,11,5,10,4], cost = [4,7,6,3,7,6,4])