# -*- coding: utf-8 -*-
# author： caoji
# datetime： 2022-09-06 22:59 
# ide： PyCharm
# 给你 n 个包裹，你需要把它们装在箱子里，每个箱子装一个包裹。总共有 m 个供应商提供 不同尺寸 的箱子（每个规格都有无数个箱子）。如果一个包裹的尺寸 小于
# 等于 一个箱子的尺寸，那么这个包裹就可以放入这个箱子之中。
#
#  包裹的尺寸用一个整数数组 packages 表示，其中 packages[i] 是第 i 个包裹的尺寸。供应商用二维数组 boxes 表示，其中
# boxes[j] 是第 j 个供应商提供的所有箱子尺寸的数组。
#
...
#
#
#  请你选择 最优 箱子供应商，使得 总浪费空间最小 。如果 无法 将所有包裹放入箱子中，请你返回 -1 。由于答案可能会 很大 ，请返回它对 10⁹ + 7
#  取余 的结果。
#
#
#
...
#
#
#  Related Topics 数组 二分查找 前缀和 排序 👍 29 👎 0
import bisect
...
class Solution:
    def minWastedSpace(self, packages: List[int], boxes: List[List[int]]) -> int:
        ...
        for box in boxes:
            ...
            tmp_sum = 0
            for size in box:
                ctr = bisect.bisect_right(packages, size)
                tmp_sum += (ctr - ptr) * size
                ptr = ctr
            res = min(res, tmp_sum - sum_)
