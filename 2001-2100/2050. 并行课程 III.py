...
#
#
#  Related Topics 图 拓扑排序 动态规划 👍 23 👎 0
import collections
...
class Solution:
    def minimumTime(self, n: int, relations: List[List[int]], time: List[int]) -> int:
        ...
        for i in range(len(relations)):
            relations[i][0] -= 1
            relations[i][1] -= 1
            g[relations[i][0]].add(relations[i][1])
        ...
        while 1:
            tree = []
            for node in T:
                ma_lst[node] += time[node]
                for nxt in g[node]:
                    ma_lst[nxt] = max(ma_lst[node], ma_lst[nxt])
                    indegree[nxt] -= 1
                    if indegree[nxt] == 0:
                        tree.append(nxt)
            if not tree:
...
print(Solution().minimumTime(2,[[2,1]],[1000,1000]))