...
#
#
#  Related Topics 贪心 字符串 回溯 计数 枚举 👍 22 👎 0
import collections
import itertools
...
class Solution:
    def longestSubsequenceRepeatedK(self, s: str, k: int) -> str:
        ...
        cand = ''.join(x * (c[x] // k) for x in sorted(c, reverse=True))
        for length in range(len(cand), 0, -1):
            for word in map(''.join, itertools.permutations(cand, length)):
                ss = iter(s)
                if all(x in ss for x in word*k):
                    return word
        return ''
