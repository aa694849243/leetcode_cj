...
class Solution:
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        if endWord not in wordList:
            return []
        if beginWord not in wordList:  # 如果startword不在wordlist则加进去
        ...
        for word in wordList:
            for i in range(len(beginWord)):
                match = word[:i] + '_' + word[i + 1:]  # 将只有一个字符不同的word分配到不同的桶里
                buckets[match].append(map1[word])
        # 根据桶做邻接表，buckets2为邻接表
        buckets2 = defaultdict(list)
        values = buckets.values()  # 将每个桶都拿出来，键值我们不关注，所以可以忽略
        for i in values:
            for j in i:
                buckets2[j].extend(i)  # 同一个桶里面的word相邻，据此每次拿一个word出来，除了该word，其他值都是该word的邻接点
                buckets2[j].remove(j)
        cost = [float('inf')] * stop  # cost表示从beginword到i代表的我word的转换次数
        res = []
        cost[map1[beginWord]] = 0  # startword不需要转换，故转换数为0

        def bfs(vi):
            qu = deque()  # 创建一个队列，保存路径
            qu.append([vi])  # 将只有初始word的路径入栈
            while qu:
                a = qu.popleft()
                for i in buckets2[a[-1]]:  # 路径最后一个word为当前节点，往后寻找
                    if cost[a[-1]] >= cost[map1[endWord]]:  # 当路径长度已经超过了最短到达endword路径，直接return，不再搜索
                        return
                    if cost[i] >= cost[a[-1]] + 1:  # 只有cost[i]转换次数为最短时可以借助往后遍历
                        cost[i] = cost[a[-1]] + 1
                        qu.append(a + [i])
                        if i == map1[endWord]:  # 找到一条最短路径，存入res
                            res.append(a + [i])
