...
class Solution:
    def unhappyFriends(self, n: int, preferences: List[List[int]], pairs: List[List[int]]) -> int:
        def check(x, y):
            index_1 = preferences[x].index(y)
            index_2 = preferences[y].index(x)
            ans = 0
            for o in preferences[x][:index_1]:
                o_ = li[o]
                index_3 = preferences[o].index(o_)
                index_4 = preferences[o].index(x)
                if index_4 < index_3:
                    ans += 1
                    break
            for o in preferences[y][:index_2]:
                o_ = li[o]
                index_3 = preferences[o].index(o_)
                index_4 = preferences[o].index(y)
                if index_4 < index_3:
                    ans += 1
                    break
            return ans
